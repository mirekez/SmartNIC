...
#
# Include this XDC only in a top that instantiates System.  The Kintex board
# top now connects Processing to System and PCIe, so create_project.tcl adds
...
# System's generated reset is held until PCIe link-up and remains static during
# normal operation.  Its release is synchronized to system_clock; exclude its
# intentional crossing into only the L2-side FIFO state.  Functional payload
# and Gray-pointer crossings remain constrained above.
set system_reset_source [get_cells -hierarchical -quiet -filter {
    IS_SEQUENTIAL == 1 && NAME =~ *host_system/system_reset_sync_reg*}]
set system_l2_fifo_regs [get_cells -hierarchical -quiet -filter {
    IS_SEQUENTIAL == 1 && (
        NAME =~ *host_system/system/*rx_cdc/write_bin_reg_reg* ||
        NAME =~ *host_system/system/*rx_cdc/write_gray_reg_reg* ||
        NAME =~ *host_system/system/*rx_cdc/read_gray_write1_reg_reg* ||
        NAME =~ *host_system/system/*rx_cdc/read_gray_write2_reg_reg* ||
        NAME =~ *host_system/system/*tx_cdc/read_bin_reg_reg* ||
        NAME =~ *host_system/system/*tx_cdc/read_gray_reg_reg* ||
        NAME =~ *host_system/system/*tx_cdc/write_gray_read1_reg_reg* ||
        NAME =~ *host_system/system/*tx_cdc/write_gray_read2_reg_reg*)}]
set_false_path -from $system_reset_source -to $system_l2_fifo_regs
