...
,   input wire reservation_invalidate_in
,   input wire[31:0] reservation_invalidate_addr_in
,   input wire mem_stall_in
,   input wire hold_in
,   input wire transaction_owner_valid_in
...
,   output wire[31:0] atomic_sc_result_out
    ...
    reg[32-1:0] reservation_physical_addr_reg;
...
;
    ...
    logic[32-1:0] reservation_physical_addr_reg_tmp;
    ...
    begin: do_memory
        ...
        if (!transaction_owner_valid_in && ((mem_split_pending_reg || atomic_pending_reg))) begin
            mem_split_pending_reg_tmp = unsigned'(1'(0));
            mem_split_write_reg_tmp = unsigned'(1'(0));
            mem_split_read_reg_tmp = unsigned'(1'(0));
            split_load_reg_tmp = unsigned'(1'(0));
            atomic_pending_reg_tmp = unsigned'(1'(0));
            atomic_op_reg_tmp = Amo_pkg::AMONONE;
            disable do_memory;
        end
        if (mem_stall_in) begin
            ...
            reservation_physical_addr_reg_tmp = reservation_physical_addr_reg;
        ...
        if (atomic_pending_reg) begin
            if (atomic_read_ready_comb) begin
                if (unsigned'(8'(atomic_op_reg)) == Amo_pkg::LR_W) begin
                    ...
                    reservation_physical_addr_reg_tmp = unsigned'(32'(dcache_read_expected_addr_in & ~'h3));
                end
                else begin
    ...
    begin: _work
        logic completing_lr;
        completing_lr=(atomic_pending_reg && (unsigned'(8'(atomic_op_reg)) == Amo_pkg::LR_W)) && atomic_read_ready_comb;
        do_memory();
        if (reservation_invalidate_in && ((((reservation_valid_reg && (unsigned'(32'(reservation_physical_addr_reg)) == ((reservation_invalidate_addr_in & ~'h3))))) || ((completing_lr && (((dcache_read_expected_addr_in & ~'h3)) == ((reservation_invalidate_addr_in & ~'h3)))))))) begin
            reservation_valid_reg_tmp = unsigned'(1'(0));
        end
        if (reset) begin
            mem_write_reg_tmp = '0;
    ...
    always_ff @(posedge clk) begin
        ...
        reservation_physical_addr_reg <= reservation_physical_addr_reg_tmp;
