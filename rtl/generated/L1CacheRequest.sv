...
,   input wire debugen_in
    ...
    // regs and combs
    L1RequestGeometryComb request_geometry_comb;
;
    L1InputRequestComb input_decode_comb;
;
    L1MemDriver mem_driver_comb;
;
...
,       SETS
    ...
    always_comb begin : request_geometry_comb_func  // request_geometry_comb_func
        ...
        request_geometry_comb.set = unsigned'(32'(((unsigned'(32'(L1CacheState___req_reg.addr))/CACHE_LINE_SIZE)) % SETS));
        request_geometry_comb.tag = unsigned'(32'(unsigned'(32'(L1CacheState___req_reg.addr))/((CACHE_LINE_SIZE*SETS))));
        request_geometry_comb.word = unsigned'(32'(((unsigned'(32'(L1CacheState___req_reg.addr)) >>> 'h2)) & ((LINE_WORDS - 'h1))));
        request_geometry_comb.refill_beat = unsigned'(32'(((unsigned'(32'(L1CacheState___req_reg.addr)) & ((CACHE_LINE_SIZE - 'h1))))/PORT_BYTES));
        request_geometry_comb.direct_cross_beat = unsigned'(1'((!L1CacheState___req_reg.cache_disable && ((((unsigned'(32'(L1CacheState___req_reg.addr)) & 'h3)) != 'h0))) && (((((unsigned'(32'(L1CacheState___req_reg.addr)) % PORT_BYTES))/'h4)) + 'h1)>=PORT_WORDS));
    end

    always_comb begin : input_decode_comb_func  // input_decode_comb_func
        input_decode_comb = 0;
        input_decode_comb.set = unsigned'(32'(((addr_in/CACHE_LINE_SIZE)) % SETS));
        input_decode_comb.cacheable = unsigned'(1'(!cache_disable_in && !((addr_in & 'h1))));
        if (((DCACHE != 'h0) && (((addr_in & 'h3)) != 'h0)) && ((((((addr_in >>> 'h2)) & ((LINE_WORDS - 'h1)))) == (LINE_WORDS - 'h1)))) begin
            input_decode_comb.cacheable = unsigned'(1'(0));
        end
    ...
    always_comb begin : mem_driver_comb_func  // mem_driver_comb_func
        mem_driver_comb = 0;
        mem_driver_comb.write = unsigned'(1'(write_in));
        mem_driver_comb.write_data = unsigned'(32'(write_data_in));
        mem_driver_comb.write_mask = unsigned'(8'(write_mask_in));
        mem_driver_comb.cache_disable = unsigned'(1'((write_in) ? (cache_disable_in) : (L1CacheState___req_reg.cache_disable)));
        mem_driver_comb.read = unsigned'(1'((L1CacheState___state_reg == L1CacheFsmState_pkg::L1_ST_REFILL) && L1CacheState___req_reg.read));
        if (((L1CacheState___state_reg == L1CacheFsmState_pkg::L1_ST_REFILL) && L1CacheState___req_reg.read) && L1CacheState___req_reg.cacheable) begin
            mem_driver_comb.addr = unsigned'(32'(((unsigned'(32'(L1CacheState___req_reg.addr)) & ~unsigned'(32'(((CACHE_LINE_SIZE - 'h1)))))) + (unsigned'(32'(L1CacheState___refill_reg.beat))*PORT_BYTES)));
        end
        else begin
            if ((L1CacheState___state_reg == L1CacheFsmState_pkg::L1_ST_REFILL) && L1CacheState___req_reg.read) begin
                if ((DCACHE != 'h0) && !L1CacheState___req_reg.cache_disable) begin
                    mem_driver_comb.addr = unsigned'(32'((request_geometry_comb.direct_cross_beat) ? (unsigned'(32'(L1CacheState___req_reg.addr))) : ((unsigned'(32'(L1CacheState___req_reg.addr)) & ~unsigned'(32'(((PORT_BYTES - 'h1))))))));
                end
                else begin
                    mem_driver_comb.addr = L1CacheState___req_reg.addr;
                end
            end
            else begin
                mem_driver_comb.addr = unsigned'(32'(addr_in));
            end
        end
    end
