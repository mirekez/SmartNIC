    ...
    always_comb begin : evict_candidate_comb_func  // evict_candidate_comb_func
        ...
        for (i='h0;i < WAYS;i=i+1) begin
            if (unsigned'(32'(evict_candidate_comb.way)) == i) begin
                evict_candidate_comb.valid = unsigned'(1'(L2CacheState___tag_q_reg[i][TAG_BITS + 'h1]));
                evict_candidate_comb.dirty = unsigned'(1'(L2CacheState___tag_q_reg[i][TAG_BITS]));
                evict_candidate_comb.tag = unsigned'(32'(unsigned'(64'(L2CacheState___tag_q_reg[i]['h0 +:TAG_BITS - 'h1 - 'h0 + 1]))));
            end
        end
        for (i='h0;i < DATA_BANKS;i=i+1) begin
            ...
            if (unsigned'(32'(evict_candidate_comb.way)) == way) begin
                evict_candidate_comb.line[word*'h20 +:32] = L2CacheState___data_q_reg[i];
            end
        end
    end
