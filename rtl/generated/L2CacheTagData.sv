...
,   input wire debugen_in
    ...
    // regs and combs
    L2HitLookupComb hit_lookup_comb;
;
    L2WordPairComb hit_write_pair_comb;
;
    L2WordPairComb fill_write_pair_comb;
;
    logic[PORT_BITWIDTH-1:0] cross_read_data_comb;
;
    logic[((((((((ADDR_BITS - $clog2(((CACHE_SIZE/CACHE_LINE_SIZE)/WAYS))) - $clog2(CACHE_LINE_SIZE)) + 'h2) + 'h7))/'h8))*'h8)-1:0] tag_write_data_comb;
;
    logic[PORT_BITWIDTH-1:0] read_data_comb[CPU_PORTS];
...
,       ((CACHE_SIZE/CACHE_LINE_SIZE)/WAYS)
    ...
    Axi4WriteAddress32_4[8-1:0] L2CacheState___slave_aw_reg_tmp;
    Axi4WriteAddress32_4[8-1:0] L2CacheState___slave_aw_seen_reg_tmp;
    Axi4ReadAddress32_4[8-1:0] L2CacheState___slave_ar_seen_reg_tmp;
    ...
    always_comb begin : hit_lookup_comb_func  // hit_lookup_comb_func
        logic[31:0] i;
        logic[31:0] way;
        ...
        for (i='h0;i < WAYS;i=i+1) begin
            if (L2CacheState___lookup_tag_reg[i][(TAG_BITS + 'h1)] && (L2CacheState___lookup_tag_reg[i]['h0 +:(TAG_BITS - 'h1) - 'h0 + 1] == L2CacheRequest___request_geometry_comb.tag)) begin
                hit_lookup_comb.hit = unsigned'(1'(1));
                hit_lookup_comb.way = unsigned'(32'(i));
            end
        end
        for (i='h0;i < DATA_BANKS;i=i+1) begin
            way=i/LINE_WORDS;
            word_index=i % LINE_WORDS;
            if (hit_lookup_comb.hit && (unsigned'(32'(hit_lookup_comb.way)) == way)) begin
                word_data=unsigned'(32'(L2CacheState___lookup_data_reg[i]));
                if (L2CacheRequest___request_geometry_comb.word == word_index) begin
                    hit_lookup_comb.aligned_word = unsigned'(32'(word_data));
                    hit_lookup_comb.read_word |= word_data >>> ((_byte*'h8));
                end
                if ((unsigned'(32'(L2CacheRequest___request_geometry_comb.word)) + 'h1) == word_index) begin
                    hit_lookup_comb.aligned_next_word = unsigned'(32'(word_data));
                    if (_byte != 'h0) begin
                        hit_lookup_comb.read_word |= word_data <<< (('h20 - (_byte*'h8)));
                    end
                end
                if (word_index>=(unsigned'(32'(L2CacheRequest___request_geometry_comb.beat))*PORT_WORDS) && (word_index < (((unsigned'(32'(L2CacheRequest___request_geometry_comb.beat)) + 'h1))*PORT_WORDS))) begin
                    ...
                    hit_lookup_comb.beat[beat_word*'h20 +:32] = L2CacheState___lookup_data_reg[i];
                end
            end
        end
    end

    always_comb begin : hit_write_pair_comb_func  // hit_write_pair_comb_func
        logic[31:0] i;
        logic[31:0] _byte;
        ...
        hit_write_pair_comb.word = unsigned'(32'(((unsigned'(32'(hit_lookup_comb.aligned_word)) & ~word_mask)) | ((word_data & word_mask))));
        hit_write_pair_comb.next_word = unsigned'(32'(((unsigned'(32'(hit_lookup_comb.aligned_next_word)) & ~next_word_mask)) | ((next_word_data & next_word_mask))));
    end

    always_comb begin : fill_write_pair_comb_func  // fill_write_pair_comb_func
        logic[31:0] i;
        logic[31:0] _byte;
        logic[31:0] word;
        ...
        fill_write_pair_comb.word = unsigned'(32'(((old_word & ~word_mask)) | ((word_data & word_mask))));
        fill_write_pair_comb.next_word = unsigned'(32'(((old_next_word & ~next_word_mask)) | ((next_word_data & next_word_mask))));
    end
    ...
    always_comb begin : cross_read_data_comb_func  // cross_read_data_comb_func
        ...
        logic[31:0] high;
        logic[31:0] data;
        cross_read_data_comb = L2CacheState___cross_low_reg;
        _byte=unsigned'(32'(L2CacheState___req_reg.addr)) & 'h3;
    ...
    always_comb begin : tag_write_data_comb_func  // tag_write_data_comb_func
        if (L2CacheState___state_reg == L2CacheFsmState_pkg::ST_INIT) begin
            ...
            tag_write_data_comb = (((unsigned'(64'('h1)) <<< ((TAG_BITS + 'h1)))) | ((unsigned'(64'(L2CacheState___req_reg.write)) <<< TAG_BITS))) | unsigned'(64'(L2CacheRequest___request_geometry_comb.tag));
        end
    end
    ...
    always_comb begin : read_data_comb_func  // read_data_comb_func
        ...
        for (index='h0;index < CPU_PORTS;index=index+1) begin
            read_data_comb[index] = (L2CacheState___response_reg[CPU_RESPONSE_BASE + index].valid) ? (L2CacheState___response_reg[CPU_RESPONSE_BASE + index].r.data) : ('h0);
        end
    end
