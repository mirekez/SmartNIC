...
,   output wire[31:0] debug_last_addr_out
    ...
    parameter  PTE_A = 'h40;
    parameter  PTE_D = 'h80;
    parameter  ST_IDLE = 64'h0;
    parameter  ST_READ_L1 = 64'h1;
    parameter  ST_READ_L0 = 64'h2;
    parameter  ST_FAULT = 64'h3;
...
,       input logic level1
                ...
                if (state_reg == ST_READ_L1) begin
                    ...
                    if (state_reg == ST_READ_L0) begin
                        ...
                        if (state_reg == ST_FAULT) begin
                            if ((!translation_enabled_comb || !(((read_in || write_in) || execute_in))) || (vaddr_in != unsigned'(32'(req_vaddr_reg)))) begin
                                fault_reg_tmp = '0;
                                state_reg_tmp = ST_IDLE;
                            end
                        end
    ...
    always_comb begin : busy_comb_func  // busy_comb_func
        logic current_walker_fault;
        busy_comb=0;
        current_walker_fault=fault_reg && (unsigned'(32'(req_vaddr_reg)) == vaddr_in);
        if (((translation_enabled_comb && (((read_in || write_in) || execute_in))) && !hit_comb) && !current_walker_fault) begin
            busy_comb=1;
        end
