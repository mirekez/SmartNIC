...
using Dma = MasterDMA<64, HOST_DATA_WIDTH, 4, 16>;
constexpr size_t HOST_BYTES = HOST_DATA_WIDTH / 8;
...
class MasterDmaTest
{
...
#if HOST_AXI4
    Axi4Responder<4, HOST_DATA_WIDTH> host = {};
    uint64_t pending_aw = 0;
    bool have_aw = false;
    bool snap_awvalid = false;
    uint64_t snap_awaddr = 0;
    bool snap_wvalid = false;
    logic<HOST_DATA_WIDTH> snap_wdata = 0;
    logic<HOST_BYTES> snap_wstrb = 0;
    ...
    logic<HOST_DATA_WIDTH> host_readdata = 0;
    bool host_readdatavalid = false;
    bool snap_write = false;
    bool snap_read = false;
    uint64_t snap_address = 0;
    logic<HOST_DATA_WIDTH> snap_writedata = 0;
    logic<HOST_BYTES> snap_byteenable = 0;
...
#define HOST_VALUE(signal) host_value_##signal()
    ...
    logic<HOST_DATA_WIDTH> host_value_wdata()
    {
#ifdef VERILATOR
        return copy_from_verilator<logic<HOST_DATA_WIDTH>>(
            dut.host___05Fwdata_out);
#else
        return dut.host.wdata_out();
#endif
    }
    logic<HOST_BYTES> host_value_wstrb()
    {
#ifdef VERILATOR
        return copy_from_verilator<logic<HOST_BYTES>>(
            dut.host___05Fwstrb_out);
#else
    ...
    logic<HOST_DATA_WIDTH> host_writedata_value()
    {
#ifdef VERILATOR
        return copy_from_verilator<logic<HOST_DATA_WIDTH>>(
            dut.host_out___05Fwritedata_out);
#else
        return dut.host_out.writedata_in();
#endif
    }
    logic<HOST_BYTES> host_byteenable_value()
    {
#ifdef VERILATOR
        return copy_from_verilator<logic<HOST_BYTES>>(
            dut.host_out___05Fbyteenable_out);
#else
    ...
    void update_host_after_edge()
    {
        ...
        if (snap_wvalid && host.w.ready) {
            ...
            for (uint32_t byte = 0; byte < HOST_BYTES; ++byte) {
